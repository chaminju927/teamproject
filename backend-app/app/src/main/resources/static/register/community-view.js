...
/*//게시글,댓글 데이터 가져오기
const fetch1 = fetch(`http://localhost:8080/community/${no}`);
const fetch2 = fetch(`http://localhost:8080/recomment/${no}`);
...
$.ajax({
  url: `http://localhost:8080/community/${no}`,
  method: 'GET',
  dataType: 'json'
})
.done(function(communityData) {
  $.ajax({
    url: `http://localhost:8080/recomment/${no}`,
    method: 'GET',
    dataType: 'json'
  })
